...
def _text_url(merchant_id: int = 42) -> str:
    return f"/api/v1/order/{merchant_id}"
...
def test_text_order_endpoint_returns_schema(monkeypatch) -> None:
    import app.api.v1.order as order_module
    from app.schemas.order import OrderPipelineSchema, OrderResponse, OrderSplit
    ...
    def fake_pipeline(request: OrderPipelineSchema):
        captured["merchant_id"] = request.merchant_id
        captured["transcript"] = request.transcript
        captured["location"] = request.customer_location
        return OrderResponse(
            merchant_id=request.merchant_id,
            total_order_cost=18.75,
    ...
    response = TestClient(app).post(
        _text_url(42),
        json={"transcript": "I need 5 boxes of nitrile gloves", "lat": 30.0, "lon": 31.0},
    )
    ...
    assert body["splits"][0]["sku"] == "NG-M"
    assert captured["merchant_id"] == 42
    assert captured["transcript"] == "I need 5 boxes of nitrile gloves"
    assert captured["location"] == (30.0, 31.0)


def test_text_order_endpoint_without_location(monkeypatch) -> None:
    import app.api.v1.order as order_module
    from app.schemas.order import OrderPipelineSchema, OrderResponse
    ...
    response = TestClient(app).post(_text_url(42), json={"transcript": "I need gloves"})

    assert response.status_code == 200
    assert captured["location"] is None


def test_text_order_endpoint_partial_location() -> None:
    response = TestClient(app).post(_text_url(42), json={"transcript": "gloves", "lat": 30.0})
    assert response.status_code == 422


def test_text_order_endpoint_empty_transcript() -> None:
    response = TestClient(app).post(_text_url(42), json={"transcript": "   "})
    assert response.status_code == 422


def test_text_order_endpoint_missing_transcript() -> None:
    response = TestClient(app).post(_text_url(42), json={})
    assert response.status_code == 422


def test_text_order_endpoint_non_integer_merchant() -> None:
    response = TestClient(app).post("/api/v1/order/abc", json={"transcript": "gloves"})
    assert response.status_code == 422


def test_text_order_endpoint_missing_merchant_path() -> None:
    response = TestClient(app).post("/api/v1/order", json={"transcript": "gloves"})
    assert response.status_code == 404
